#!/usr/bin/env python3
# rt_spiral_origin_viewer_AB_fullclosure_staplar_v1.py
#
# Rhythm Theory full closure viewer.
#
# Main closure:
...
#
# Visual policy:
#   - same visual family as the locked 210-closure script
#   - RP start view by default
#   - compact panel/table for locked/found values
#   - four bars side by side
#   - one single "arming" bracket on one 21-window
#   - PP bracket on the geometry body
#   - RP arrow to the top readout surface
#
# Addendum policy:
#   tp-p is shown as pedagogical/readout structure.
#   The final internal form of one tp-p turn is not locked by this script.
...
def draw_side_bracket(ax, x, y, z0, z1, label="", color="0.15",
    ...
    side="right"  -> bracket opens to +x
    side="left"   -> bracket opens to -x
    """
    zmin, zmax = min(z0, z1), max(z0, z1)
    sgn = 1.0 if side == "right" else -1.0

    ax.plot([x, x+sgn*arm], [y, y], [zmax, zmax], color=color, lw=lw, alpha=0.95)
    ax.plot([x+sgn*arm, x+sgn*arm], [y, y], [zmax, zmax-out], color=color, lw=lw, alpha=0.95)
    ax.plot([x, x], [y, y], [zmin, zmax], color=color, lw=lw, alpha=0.95)
    ax.plot([x, x+sgn*arm], [y, y], [zmin, zmin], color=color, lw=lw, alpha=0.95)
    ax.plot([x+sgn*arm, x+sgn*arm], [y, y], [zmin, zmin+out], color=color, lw=lw, alpha=0.95)

    if label:
        zlab = zmin + label_z_frac * (zmax - zmin)
        if side == "right":
            safe_text(ax, x+arm+text_dx, y, zlab, label,
...
def draw_full_bar_group(ax, x0, y0, z0, K, wave, full, dx=0.44):
    """
    Five bars placed side by side for full closure:
      black  : PrimalPlane (PP)
      blue   : 42×30 = 1260
      red    : 6×210 = 1260
      green  : 60×21 = 1260
      orange : 1260 full closure
    ...
    half_wave = wave // 2
    bars = [
        (x0 + 0*dx, y0, full,      1,                 "PrimalPlane (PP)",   "0.05",    1.45, False, 0.22),
        (x0 + 1*dx, y0, K,         full // K,         "42×30 = 1260",       "#1F77B4", 1.15, False, 0.35),
        (x0 + 2*dx, y0, beat,      full // beat,      "6×210 = 1260",       "#D62728", 1.15, False, 0.48),
        (x0 + 3*dx, y0, half_wave, full // half_wave, "60×21 = 1260",       "#2CA02C", 1.15, False, 0.61),
        (x0 + 4*dx, y0, full,      1,                 "1260 full closure",  "#E87500", 1.55, True, 0.74),
    ]
    for x, y, step, count, label, color, lw, is_master, label_z_frac in bars:
        # Move the PP label to the other side of the black bar and place it at half height.
        bar_label = "" if label == "PrimalPlane (PP)" else label
        draw_segment_ticks(
            ax, x, y, z0, step, count, bar_label,
            color=color, lw=lw, cap=0.075 if is_master else 0.070,
            tick_cap=0.055, text_dx=0.12, label_z_frac=label_z_frac
        )

        if label == "PrimalPlane (PP)":
            safe_text(
                ax,
                x - 0.12,
                y,
        ...
        # Mark exactly one 21-window on the green 60×21 bar.
        if label == "60×21 = 1260":
            z_a0 = z0 + 59 * step
            z_a1 = z0 + 60 * step
            draw_side_bracket(
...
def draw_vertical_bracket(ax, x, y, z0, z1, label, color="0.10",
                          hook=0.12, inset=0.08, lw=1.25, text_dx=0.18):
    """
    Tall outer bracket:
      ┌
      │
      └
    Used here to label the main geometry body as PrimalPlane (PP).
    """
    zmin, zmax = min(z0, z1), max(z0, z1)
    # top hook
    ax.plot([x, x+hook], [y, y], [zmax, zmax], color=color, lw=lw, alpha=0.95)
    ax.plot([x+hook, x+hook], [y, y], [zmax, zmax-inset], color=color, lw=lw, alpha=0.95)
    # spine
    ax.plot([x, x], [y, y], [zmin, zmax], color=color, lw=lw, alpha=0.95)
    # bottom hook
    ax.plot([x, x+hook], [y, y], [zmin, zmin], color=color, lw=lw, alpha=0.95)
    ax.plot([x+hook, x+hook], [y, y], [zmin, zmin+inset], color=color, lw=lw, alpha=0.95)

    safe_text(ax, x-hook-text_dx, y, 0.5*(zmin+zmax), label,
              fontsize=10, color=color, ha="right", va="center")


def add_info_panel(fig):
    """
    Compact 2D info panel with locked/found minimal values.
    """
    x0 = 0.72
    y0 = 0.84

    fig.text(x0, y0, "full closure panel", fontsize=10, color="0.08", ha="left", va="top")
    yy = y0 - 0.050
    ...
    rows = [
        ...
        ("K", "30"),
        ("wave", "42"),
        ("beat", "210 = 7×30 = 5×42 = 10×21"),
        ("full closure", "1260 = 6×210 = 42×30"),
    ]
    for key, val in rows:
        fig.text(x0, yy, key, fontsize=8.0, color="0.15", ha="left", va="center")
        fig.text(x0 + 0.14, yy, val, fontsize=8.0, color="0.15", ha="left", va="center")
        yy -= 0.024


def validate_args(args):
    if args.full <= 0:
        raise ValueError("--full måste vara > 0")
    if args.K <= 0:
    ...
    beat = lcm_int(args.K, args.wave)
    if args.full % beat != 0:
        print(f"VARNING: full={args.full} är inte jämnt delbart med beat={beat}", file=sys.stderr)
    if args.full % args.K != 0 or args.full % args.wave != 0 or args.full % (args.wave//2) != 0:
        print("VARNING: full är inte jämnt delbar i 30/42/21", file=sys.stderr)
...
def draw(ax, args):
    ...
    # Over one 1260 full closure:
    #   e_main_turns = 1260 / 30 = 42
    ...
    if not args.no_p:
        p_samples = max(args.samples, 22000)
        (t, xA, yA, zA), (_, xB, yB, zB) = rotor_pair_closure(
            Rp, ap,
        ...
        ax.plot3D(xA, yA, zA, lw=0.44, label="p_A", alpha=0.56)
        ax.plot3D(xB, yB, zB, lw=0.41, label="p_B", alpha=0.42)
        plotted = True
    ...
    # show the 6 × 210 segmentation inside the body
    if not args.no_segments:
        for n in range(7):
            z = zmin + n * beat
            draw_circle(ax, scene_r*1.03, z, color="0.20", lw=0.72, alpha=0.25, ls="-" if n in (0,6) else "--")
    ...
    # labels on geometry body
    draw_label_arrow(
        ax,
        text_x=-scene_r*1.50,
        text_y=scene_r*1.06,
        target_x=-scene_r*0.72,
        target_y=0.0,
        z=zmax,
        label="tp-e",
        color="0.05",
    ...
    # RP arrow to top surface
    draw_label_arrow(
        ax,
        text_x=-scene_r*0.92,
        text_y=scene_r*1.06,
        target_x=-scene_r*0.30,
        target_y=scene_r*0.98,
        z=zmax,
        label="RealPlane (RP)",
        color="0.05",
    )
    ...
    ax.set_xlim(-scene_r*2.62, scene_r*2.18)
    ax.set_ylim(-scene_r*2.05, scene_r*2.15)
    ...
    safe_text(ax, -scene_r*1.62, scene_r*1.55, zmax, "Rhythm Theory Full Closure",
              fontsize=11, color="0.08", ha="left", va="center")
    safe_text(ax, -scene_r*1.62, scene_r*1.40, zmax, "1260 = 6×210 = 42×30",
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="RT full closure viewer: 1260 = 6×210 = 42×30 = 60×21."
    )
    ...
    ap.add_argument("--full", type=int, default=FULL_DEFAULT)
    ...
    ap.add_argument("--z-aspect", type=float, default=3.1)
