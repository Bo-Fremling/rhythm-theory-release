...
- This does NOT change quark proxy values; it validates that the preferred-rule
  is non-degenerate and that nearby rule-variants would change the preferred
  when that contrast is actually available.
...
- For each NEG variant where it is *meaningful* and non-degenerate, the NEG-picked
  denominator MUST differ from canonical, and the corresponding (p,d) MUST also
  exist as a candidate.

Soft / informational
- If a NEG variant collapses to the same denominator as canonical, that NEG is
  recorded as DEGENERATE / not informative, but it is not treated as a hard fail.
  This avoids rejecting a valid canonical result merely because the contrast
  ladder no longer separates under the current full-scan canonical regime.
...
def main() -> int:
    ...
    for key in ("m_u_over_m_c", "m_c_over_m_t", "m_d_over_m_s", "m_s_over_m_b"):
        blk = cs.get(key)
        if not isinstance(blk, dict):
            hard_issues.append({"type": "missing_block", "key": key})
            continue
        cands = blk.get("candidates")
        if not isinstance(cands, list) or not cands:
            hard_issues.append({"type": "missing_candidates", "key": key})
            continue
        pd = _canonical_pd(key)
        if pd is None:
            hard_issues.append({"type": "missing_preferred_pd", "key": key})
            continue
        p0, d0 = pd
        if not _has_pd(cands, p0, d0):
            hard_issues.append({"type": "canonical_pd_not_in_candidates", "key": key, "p": p0, "d": d0})
        canonical[key] = {"p": p0, "d": d0}

    neg = []

    for sec, qvals, key in (
        ("u", u_q_vals, "m_u_over_m_c"),
        ("d", d_q_vals, "m_d_over_m_s"),
    ):
        if len(qvals) >= 2 and key in canonical:
            ...
            degenerate = d_alt == d_can
            if degenerate:
                soft_issues.append({
                    "type": "neg_gap12_degenerate",
                    "sector": sec,
                    "key": key,
                    "canon_d": d_can,
                    "alt_d": d_alt,
            ...
            neg.append({
                ...
                "alt": {"p": p_can, "d": d_alt},
                "q_vals": qvals,
                "expect": "DIFF (non-degenerate)" if not degenerate else "DEGENERATE / NOT INFORMATIVE",
            })
    ...
    out = {
        ...
        "validation": {
            ...
            "note": "NEG controls are structural. Degenerate NEG variants are recorded but not treated as fatal; only missing canonical candidate structure or missing non-degenerate alternative candidates remain hard fails.",
        },
    }
